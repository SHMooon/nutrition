...
precalc_saved_diabetes <- vv(red_diabetes, var_CV, n_years)*
                            vv(cost_diabetes, var_CV, n_years)


precalc_saved_obesities <- vv(red_obesity, var_CV, n_years)*
                            vv(cost_obesity, var_CV, n_years)

precalc_saved_cancer <- vv(red_cancer, var_CV, n_years)*
                          vv(cost_cancer, var_CV, n_years)

precalc_tax_revenue <- vv(revenue_tax, var_CV, n_years)

    ##should i make a precalc total health care cost - saved cost - tax revenue  for with SSB
    ## and withouth will be only total health care cost
...
##cost for intervention##
  ...
  # health care cost 
  cost_diabetes <-vv(total_diabetes, var_CV, n_years)*
    vv(cost_diabetes, var_CV, n_years)
  
  cost_obesities <- vv(total_obesity, var_CV, n_years)*
    vv(cost_obesity, var_CV, n_years)
  
  cost_cancer <- vv(total_cancer, var_CV, n_years)*
    vv(cost_cancer, var_CV, n_years)
...
######
  implementation_SSB_tax_Di <- 
    as.numeric(intervention_cost) * precalc_saved_diabetes
  implementation_SSB_tax_OB <-
    as.numeric(intervention_cost) * precalc_saved_obesities
  implementation_SSB_tax_Ca <- 
    as.numeric(intervention_cost) * precalc_saved_cancer
  
  health_care_SSB <- implementation_SSB_tax_Di +implementation_SSB_tax_OB+
    implementation_SSB_tax_Ca
  
  total <- 
  as.numeric(intervention_cost) * total_health_care_cost
  ...
  if (decision_implementation_SSB_Tax){
    net_health_care_cost <-     health_care_SSB - implementation_SSB + imple_tax_revenue
    # save result in result_imple (result implementation):
    result_imple <- net_health_care_cost}
  
  
  if (!decision_implementation_SSB_Tax){
    net_health_care_cost <- 0 - intervention_cost
